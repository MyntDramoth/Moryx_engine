...
void Swapchain::reload_buffers(UINT width, UINT height) {
    ...
    if(FAILED(hres)) {
       MORYX_ERROR("Failed to get Swap Chain buffer!");
    }
    ...
    if(FAILED(hres)) {
       MORYX_ERROR("Failed to create Render Target View!");
    }
    ...
    if(FAILED(hres)) {
        MORYX_ERROR("Failed to create Depth Buffer!");
    }

    hres = d3_device->CreateDepthStencilView(back_buffer,NULL,&stencil_view);
    if(FAILED(hres)) {
        MORYX_ERROR("Failed to create Depth Stencil View!");
    }
    back_buffer->Release();
